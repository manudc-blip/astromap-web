...
import { downloadBlob, downloadTextFile, inlineSvgImages, svgTextToPngBlob } from "../lib/export";
import AstroSidebar, {
...
export default function AstroMapPage() {
  ...
  const runGraphicExport = async () => {
    ...
    try {
      ...
      if (exportKind === "png") {
        const svgWithEmbeddedGlyphs = await inlineSvgImages(current);
        const blob = await svgTextToPngBlob(svgWithEmbeddedGlyphs, 2);
        downloadBlob(`astromap-${activeTab}.png`, blob);
        setExportDialogOpen(false);
        return;
      }
    ...
    <div className="astromap-app">
      <div className="astromap-sidebar-shell">
        <AstroSidebar
          ...
          onToggleTimeRef={() => setForm((prev) => toggleDisplayedTimeRef(prev))}
      ...
      {exportDialogOpen && activeTab !== "interpretation" ? (
        <div
          onClick={() => {
          ...
          >
            ...
            <label style={{ display: "block", marginBottom: 8 }}>
              <input
                ...
                checked={exportKind === "png"}
                onChange={() => setExportKind("png")}
              />{" "}
              {language === "en" ? "PNG image" : "Image PNG"}
            </label>
