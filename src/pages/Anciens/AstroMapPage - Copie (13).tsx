...
export default function AstroMapPage() {
  ...
  const handleSelectDnSuggestion = useCallback(
    async (item: SidebarSuggestion) => {
      ...
      const rawDnName = (rec.displayName || "").trim();
      const dnParts = rawDnName.split(/\s+/);

      let prettyDnTitle = rawDnName;

      if (dnParts.length >= 2) {
        const last = dnParts[0];
        const first = dnParts.slice(1).join(" ");
        prettyDnTitle = `${first} ${last}`;
      }

      setCurrentThemeOwnerTitle(prettyDnTitle);
  ...
  const runInterpretationExport = async () => {
    ...
    try {
      ...
      const [eclipticSvg, retSvg, aspectsSvg] = await Promise.all([
        inlineSvgImages(ecliptic),
        inlineSvgImages(ret),
        inlineSvgImages(aspects),
      ]);
  ...
  const runGraphicExport = async () => {
    ...
    try {
      ...
      const svgMarkup = await inlineSvgImages(current);
  ...
  const currentContent = useMemo(() => {
    ...
    if (!owner || !raw.includes("<svg")) return raw;
        ...
        >
          ...
          <div className="astromap-nav-arrows">
            <button
              ...
              title={isEn ? "Previous tab" : "Onglet précédent"}
            >
              ❮
            </button>
            ...
            <button
              ...
              title={isEn ? "Next tab" : "Onglet suivant"}
            >
              ❯
            </button>
          </div>
